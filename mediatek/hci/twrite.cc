...
class BTSnoopBootBackupHelper {
  public:
   ...
   std::string Backup(const base::FilePath& to_path) {
     ...
     LOG_INFO(LOG_TAG, "%s try backup from %s to %s",
     ...
     for (base::FilePath name = file_enum.Next(); !name.empty(); name = file_enum.Next()) {
       if (base::StartsWith(GetFileName(name), kLogFilePrefix, base::CompareCase::SENSITIVE)) {
         if (base::DeleteFile(name, false)) {
           LOG_WARN(LOG_TAG, "%s %s is deleted", __func__, name.value().c_str());
         }
       }
     }
   }
  ...
  private:
   ...
   bool MoveBootLogs(const base::FilePath& file_path,
     ...
     LOG_INFO(LOG_TAG, "%s try move from %s to %s", __func__,
   ...
   std::string BackupBootLogs(const base::FilePath& from_dir,
     ...
     LOG_INFO(LOG_TAG, "%s: the last one is %s",
...
class BtSnoopHelper {
  public:
   ...
   void Stop() {
     ...
     FlushData();
     LOG_INFO(LOG_TAG, "%s: BtSnoopHelper Stop", __func__);
     {
...
class BtSnoopImpl : public BtSnoopSubject {
 public:
  ...
  void StartUp(const BtSnoopStartUpArg& arg) override {
    if (is_started_) {
      LOG_INFO(LOG_TAG, "%s: BtSnoopImpl ignore dup StartUp", __func__);
      return;
    }
    std::lock_guard<std::mutex> lock(btsnoop_mutex_);
    LOG_INFO(LOG_TAG, "%s: BtSnoopImpl StartUp", __func__);
    Setup(arg.base_dir_);
  ...
  void Stop() override {
    if (!is_started_) {
      LOG_INFO(LOG_TAG, "%s: BtSnoopImpl ignore dup Stop", __func__);
      return;
    }
    std::lock_guard<std::mutex> lock(btsnoop_mutex_);
    LOG_INFO(LOG_TAG, "%s: BtSnoopImpl Stop", __func__);
    if (bt_snoop_helper_) {
  ...
  public:
    ...
    void OnBtLogStateChanged(const BluetoothLogSetting* setting) override {
      ...
      if (is_boot_logging_) {
        ...
        LOG_INFO(LOG_TAG, "%s boot: new state: %s vs current state: %s.",
            __func__,
        ...
        LOG_INFO(LOG_TAG, "%s normal: new state: %s vs current state: %s.",
            __func__,
            setting->IsHciSnoopEnabled() ? "on" : "off",
            btsnoop_->IsLogging() ? "on" : "off");
        if (btsnoop_->IsLogging() !=
          ...
          LOG_INFO(LOG_TAG, "%s ignore for duplicated request %s.",
              __func__,
              setting->IsHciSnoopEnabled() ? "on" : "off");
        }
      }
    ...
    void StartUp(const BtSnoopStartUpArg& arg) override {
      if (!is_boot_logging_) {
        if (!is_need_backup_boot_logs_) {
          LOG_INFO(LOG_TAG, "%s backup already", __func__);
          btsnoop_->StartUp(arg);
        } else {
          LOG_INFO(LOG_TAG, "%s do backup", __func__);
          btsnoop_->Stop();
          {
            std::lock_guard<std::mutex> lock(
        ...
        LOG_INFO(LOG_TAG, "%s boot", __func__);
    ...
    void StartInitLogTimer() {
      ...
      if (init_log_timer_) {
        alarm_set(init_log_timer_, kInitLogCancelTimeoutMs,
            [](void* context) {
                CHECK(NULL != context);
                LOG_INFO(LOG_TAG, "%s, no log command, to stop", __func__);
